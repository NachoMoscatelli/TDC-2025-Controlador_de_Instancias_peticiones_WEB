...
class SystemManager:
    ...
    def scale(self, pid_signal):
        ...
        # --- Umbrales de DESESCALADO (latencia BAJA -> error POSITIVO -> señal POSITIVA) ---
        SCALE_DOWN_MILD_THRESHOLD = 0.08     # Error < -47ms (Latencia < 153ms)
        SCALE_DOWN_MODERATE_THRESHOLD = 0.12   # Error < -70ms (Latencia < 130ms)
        SCALE_DOWN_SEVERE_THRESHOLD = 0.16     # Error < -94ms (Latencia < 106ms)
